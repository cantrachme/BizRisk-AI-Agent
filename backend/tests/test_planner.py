...
def test_state_updates_in_langgraph_flow():
    ...
    # Assert correct state transitions and loop updates
    assert output_state["planner_loop_count"] == 1
    assert len(output_state["pending_tasks"]) == 1
    assert output_state["pending_tasks"][0].task_type == "GST_VERIFICATION"
    assert output_state["status"] == "PENDING_RESEARCH"
